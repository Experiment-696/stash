...
This fork's spotlight feature is first-class multi-user access. A fresh install creates an Admin account through the normal setup flow; additional accounts are managed in Stash and sign in through the built-in login screen. Admin, Moderator, and User roles keep server administration separate from day-to-day library use, while each account retains its own homepage, saved filters, ratings, O-counter state, and theme choice. Shared ratings expose a global average without replacing anyone's personal rating.

The library also includes an evolving Cam Shows workspace. Shows remain separate from ordinary Scenes, and Cam Models remain separate from Performers, with site-specific aliases for identities that change between platforms. CamGirlFinder integration can identify an existing model and attach matching account details; an explicit profile action can then import public profile fields such as a missing image, age, location, and structured social links while preserving information already entered in Stash.
